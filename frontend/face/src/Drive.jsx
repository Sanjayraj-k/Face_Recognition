...
import { Upload, Search, User, LogOut, Camera, Image, RefreshCw, Eye, EyeOff, AlertCircle, CheckCircle, Loader2, Star, Sparkles, Folder, Download, Google } from 'lucide-react';

// Define the absolute URL for your backend API
const API_BASE_URL = 'http://localhost:5000/api';

// Google OAuth2 Configuration - Replace with your actual client ID
const GOOGLE_CLIENT_ID = 'YOUR_GOOGLE_CLIENT_ID.apps.googleusercontent.com'; // Replace with your client ID
const GOOGLE_REDIRECT_URI = window.location.origin + '/oauth2callback'; // Adjust if needed
const GOOGLE_SCOPES = 'https://www.googleapis.com/auth/drive.metadata.readonly https://www.googleapis.com/auth/drive.file';
...
const SnapIDApp = () => {
  ...
  const [folderIdForm, setFolderIdForm] = useState({ folderId: '' });
  ...
  // Handle Google OAuth callback (check URL hash on mount)
  useEffect(() => {
  ...
  const handleGoogleSignOut = () => {
    ...
    setDriveFiles([]);
    showMessage('info', 'Google Drive disconnected.');
  };

  const fetchDriveFiles = async () => {
    if (!googleAccessToken || !driveFolderId) {
      showMessage('error', 'Please connect Google Drive and enter a folder ID.');
      return;
    }

    setFetchingDriveFiles(true);
    try {
      const response = await fetch(`${API_BASE_URL}/fetch_drive_files`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ folder_id: driveFolderId, access_token: googleAccessToken }),
        credentials: 'include'
      });
      ...
      if (response.ok && data.success) {
        ...
        showMessage('success', `Fetched ${data.files.length} files from Google Drive.`);
      } else {
        showMessage('error', data.message || 'Failed to fetch files from Google Drive.');
      }
    } catch (error) {
      console.error('Error fetching Drive files:', error);
      showMessage('error', 'Network error while fetching from Google Drive.');
    }
    setFetchingDriveFiles(false);
  };

  const handleUploadSimilarPhotos = async () => {
    if (!driveFiles.length) {
      showMessage('error', 'No Drive files to upload.');
      return;
    }
    ...
    // Append Drive file metadata; backend will download using access_token
    driveFiles.forEach((file, index) => {
      formData.append('drive_files', JSON.stringify(file));
    });

    try {
      const response = await fetch(`${API_BASE_URL}/upload_drive_album`, {
        method: 'POST',
        body: formData,
        credentials: 'include'
      });
      ...
      if (response.ok && data.success) {
        ...
        await fetch(`${API_BASE_URL}/update_cache`, {
          method: 'POST',
          credentials: 'include'
        });
      } else {
  ...
  const handleAlbumUpload = async (files) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        // Trigger cache update after upload
        await fetch(`${API_BASE_URL}/update_cache`, {
          method: 'POST',
          credentials: 'include'
        });
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">Hello, {currentUser} 👋</p>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div className="bg-white/60 backdrop-blur-xl rounded-3xl shadow-xl p-8 border border-gray-200/50">
                <div className="text-center mb-6">
                  <div className="w-20 h-20 bg-gradient-to-r from-green-100 to-emerald-100 rounded-3xl mx-auto mb-4 flex items-center justify-center shadow-lg">
                    <Folder className="w-10 h-10 text-green-600" />
                  </div>
                  <h2 className="text-3xl font-bold text-gray-900 mb-2">Connect Google Drive</h2>
                  <p className="text-gray-600 text-lg">Fetch your photo dataset from a Drive folder</p>
                </div>

                {!isGoogleSignedIn ? (
                  <button
                    onClick={handleGoogleSignIn}
                    className="w-full bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 text-white py-4 rounded-2xl font-medium hover:shadow-lg hover:shadow-purple-500/25 transition-all transform hover:scale-105 flex items-center justify-center gap-3 group"
                  >
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      ...
                      <input
                        type="text"
                        value={folderIdForm.folderId}
                        onChange={(e) => setFolderIdForm({...folderIdForm, folderId: e.target.value})}
                    ...
                    <button
                      ...
                      disabled={fetchingDriveFiles || !folderIdForm.folderId}
                      className="w-full bg-gradient-to-r from-green-500 to-emerald-500 text-white py-4 rounded-2xl font-medium hover:shadow-lg hover:shadow-green-500/25 transition-all transform hover:scale-105 disabled:opacity-50 disabled:transform-none flex items-center justify-center gap-3 group"
                    >
                      {fetchingDriveFiles ? (
                        <Loader2 className="w-5 h-5 animate-spin" />
                      ) : (
                      ...
                      {fetchingDriveFiles ? 'Fetching Files...' : 'Fetch Files from Folder'}
                    </button>
                  </div>
                )}
              </div>
              ...
              {driveFiles.length > 0 && (
                <div className="bg-white/60 backdrop-blur-xl rounded-3xl shadow-xl p-8 border border-gray-200/50">
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                      <h3 className="text-2xl font-bold text-gray-900">
                        Found {driveFiles.length} Files in Drive Folder
                      </h3>
                      <p className="text-gray-600">Ready to upload as your photo album</p>
                    </div>
                  </div>
                  
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-6">
                    {driveFiles.slice(0, 8).map((file, index) => (  // Show first 8 for preview
                      <div key={file.id} className="relative overflow-hidden rounded-2xl shadow-md bg-gray-100 p-4 text-center">
                  ...
                  <button
                    onClick={handleUploadSimilarPhotos}
                    disabled={loading}
                    className="w-full bg-gradient-to-r from-purple-500 to-pink-500 text-white py-4 rounded-2xl font-medium hover:shadow-lg hover:shadow-pink-500/25 transition-all transform hover:scale-105 disabled:opacity-50 disabled:transform-none flex items-center justify-center gap-3 group"
                  >
                    {loading ? (
                      <Loader2 className="w-5 h-5 animate-spin" />
                    ) : (
                    ...
                    {loading ? 'Uploading from Drive...' : 'Upload All as Similar Photos'}
                  </button>
                </div>
              )}
            </div>
              ...
              >
                ...
                <div className="relative z-10">
                  ...
                  <p className="text-gray-500 text-lg">
                    Select multiple photos • JPG, PNG supported • Max 10MB each
                  </p>
                </div>
              </div>
